...
class Tank {
	...
	get drain() {
		...
		if (simSettings.variableDrain === true) {
			if (this.RNG() > 0.56) {
				drain = this.lastDrain + (this.RNG() * range);
			} else {
				drain = this.lastDrain - (this.RNG() * range);
			}
		} else {
			drain = this.maxDrain - 1 + this.maxDrain * (this.level / 500);
		}
...
document.querySelector("#renderGraph").onclick = () => {
...
document.querySelector("#setpoint").onchange = () => {
	simSettings.setpoint = document.querySelector("#setpoint").value || 0;
};
document.querySelector("#processDelay").value = simSettings.simulationDelay;
document.querySelector("#processDelay").onchange = () => {
	simSettings.simulationDelay = Number(document.querySelector("#processDelay").value) || 1;
};
document.querySelector("#acceleration").value = simSettings.acceleration;
document.querySelector("#acceleration").onchange = () => {
...
document.querySelector("#simulationLength").onchange = () => {
	simSettings.simulationLength = document.querySelector("#simulationLength").value || 0;
};
document.querySelector("#runAutotune").onclick = () => {
...
document.querySelector("#variableSetpoint").onclick = () => {
...
document.querySelector("#variableDrain").onclick = () => {
...
document.querySelector("#ATgenerations").onchange = () => {
...
document.querySelector("#usePRNG").onclick = () => {
...
document.querySelector("#ATstabilityFactor").onchange = () => {
...
document.querySelector("#ATtrainingRoot").onchange = () => {
